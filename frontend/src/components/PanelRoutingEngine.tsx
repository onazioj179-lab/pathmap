...
export default function PanelRoutingEngine({
  state,
  updateState,
  onClose,
}: PanelRoutingEngineProps) {
    ...
    <AnimatePresence>
      ...
      >
        ...
        <div className="p-3 space-y-2">
            ...
            {['ShadowPath', 'HomeGuard', 'PathfinderX'].map(algo => (
              <button
                ...
                className={`py-1.5 text-xs font-medium touch-manipulation ${
                  state.algorithm === algo ? 'bg-white text-black' : 'bg-gray-800 text-gray-400'
                }`}
              >
          ...
          {error && <div className="bg-red-900/30 p-2 text-red-300 text-xs">{error}</div>}
              ...
              <div className="grid grid-cols-2 gap-1 text-xs">
                <div className="flex justify-between">
                  ...
                  <span className="text-white font-medium">
                    {state.routeData.distance?.toFixed(2)}
                  </span>
                </div>
                <div className="flex justify-between">
