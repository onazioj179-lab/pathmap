...
export function TimeAnalyticsPanel({ isVisible, onClose }: TimeAnalyticsPanelProps) {
  ...
  const statusClass =
    ...
    )[performanceStatus] ?? 'border-gray-300 text-gray-500';

  return (
    <div
      className={`fixed top-20 right-5 z-50 w-[400px] max-h-[80vh] overflow-y-auto v72-panel border-2 shadow-xl font-mono text-xs ${statusClass}`}
    >
      {/* Header */}
      <div className="flex items-center justify-between mb-3">
        ...
        <button
          ...
          aria-label="Close time analytics"
          title="Close"
        >
          <svg
          ...
          >
            <path
              fillRule="evenodd"
              d="M6.225 4.811a.75.75 0 011.06 0L12 9.525l4.715-4.714a.75.75 0 111.06 1.06L13.06 10.586l4.714 4.715a.75.75 0 11-1.06 1.06L12 11.646l-4.715 4.715a.75.75 0 11-1.06-1.06l4.714-4.715-4.714-4.714a.75.75 0 010-1.061z"
              clipRule="evenodd"
            />
          </svg>
        </button>
      ...
      <div className="mb-4 p-3 bg-gray-50 rounded-md">
        ...
        {metrics.isSlowDevice && (
          <div className="text-[11px] text-yellow-500">
            Slow device detected - optimizations active
          </div>
        )}
      </div>
      ...
      <MetricSection
        ...
        extra={
          <div className="text-[11px] text-gray-400 mt-1">
            Cycle Rate: {metrics.navigationCycleRate.toFixed(2)} Hz
          </div>
        }
      />
      ...
      <div className="mb-3 p-3 bg-gray-50 rounded-md">
        ...
        <div
          className={`text-[20px] font-bold ${metrics.visualizationFPS >= thresholds.visualizationFPS ? 'text-green-500' : metrics.visualizationFPS >= thresholds.visualizationFPS * 0.7 ? 'text-yellow-500' : 'text-red-500'}`}
        >
      ...
      <div className="mb-3 p-3 bg-gray-50 rounded-md">
        ...
        <div className="flex justify-between text-[11px] mb-1">
          ...
          <span
            className={
              getColorForMetric(metrics.apiLatencyAvg, thresholds.apiLatency) === '#10b981'
                ? 'text-green-500'
                : getColorForMetric(metrics.apiLatencyAvg, thresholds.apiLatency) === '#f59e0b'
                  ? 'text-yellow-500'
                  : getColorForMetric(metrics.apiLatencyAvg, thresholds.apiLatency) === '#ef4444'
                    ? 'text-red-500'
                    : 'text-gray-400'
            }
          >
        ...
        <div className="flex justify-between text-[11px] mb-1">
          ...
          <span
            className={
              getColorForMetric(metrics.apiLatencyMax, thresholds.apiLatency) === '#10b981'
                ? 'text-green-500'
                : getColorForMetric(metrics.apiLatencyMax, thresholds.apiLatency) === '#f59e0b'
                  ? 'text-yellow-500'
                  : getColorForMetric(metrics.apiLatencyMax, thresholds.apiLatency) === '#ef4444'
                    ? 'text-red-500'
                    : 'text-gray-400'
            }
          >
      ...
      {metrics.slowestOperation && (
        <div className="mb-3 p-3 rounded-md bg-red-50">
          ...
          <div className="text-[11px]">
            {metrics.slowestOperation.replace(/_/g, ' ').toUpperCase()}
          </div>
          <div className="text-[18px] font-bold text-red-500 mt-1">
            {metrics.longestRecentDelay.toFixed(0)} ms
          </div>
        </div>
      )}
      ...
      <div className="mt-4 pt-4 border-t border-gray-200">
        ...
        <div className="flex justify-between text-[11px]">
          ...
          <span>{(metrics.totalDuration / 1000).toFixed(2)} s</span>
        </div>
      </div>

      {/* Footer */}
      <div className="mt-4 text-[10px] text-gray-500 text-center">
        V32 Time Engine • Updates every 1s
      </div>
    </div>
  );
}
...
function MetricSection({
  ...
  return (
    <div className="mb-3 p-3 bg-gray-50 rounded-md">
      ...
      {current !== null && (
        <div
          className={`text-[18px] font-bold ${getColorForMetric(current, threshold) === '#10b981' ? 'text-green-500' : getColorForMetric(current, threshold) === '#f59e0b' ? 'text-yellow-500' : getColorForMetric(current, threshold) === '#ef4444' ? 'text-red-500' : 'text-gray-400'} mb-1`}
        >
      ...
      <div className="flex justify-between text-[11px] mb-0.5">
        ...
        <span
          className={
            getColorForMetric(average, threshold) === '#10b981'
              ? 'text-green-500'
              : getColorForMetric(average, threshold) === '#f59e0b'
                ? 'text-yellow-500'
                : getColorForMetric(average, threshold) === '#ef4444'
                  ? 'text-red-500'
                  : 'text-gray-400'
          }
        >
      ...
      <div className="flex justify-between text-[11px] mb-0.5">
        ...
        <span
          className={
            getColorForMetric(max, threshold) === '#10b981'
              ? 'text-green-500'
              : getColorForMetric(max, threshold) === '#f59e0b'
                ? 'text-yellow-500'
                : getColorForMetric(max, threshold) === '#ef4444'
                  ? 'text-red-500'
                  : 'text-gray-400'
          }
        >
