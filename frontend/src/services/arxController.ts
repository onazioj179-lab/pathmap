...
class ARXController {
  ...
  static get instance() { if (!this._instance) this._instance = new ARXController(); return this._instance; }
  ...
  async start(route: RouteLikeV64 | null) {
    ...
    try {
      const features = { requiredFeatures: ['local-floor'], optionalFeatures: ['anchors', 'dom-overlay'], domOverlay: { root: document.body } } as any;
      ...
      setARMode(true);
      try { uiScaleEngine.setMode('AR'); } catch {}
  ...
  setRoute(route: RouteLikeV64 | null) { this.route = route; }
}
